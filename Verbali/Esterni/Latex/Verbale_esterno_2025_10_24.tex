...
Chiarimento dubbi sul capitolato$^G$ C6 e primo contatto con il referente dell'azienda Zucchetti.

\vspace{1\baselineskip}

L'incontro è stato richiesto in particolare per discutere eventuali preferenze su framework$^G$ e stack da utilizzare e per precisare i requisiti di una possibile implementazione di un database remoto.
...
\section*{Verbale}
Abbiamo fatto una riunione su Google Meet con il Sig. Piccoli per chiarire alcuni dubbi sulle richieste del capitolato$^G$ C6.

\vspace{1\baselineskip}

\begin{itemize}
    \item Framework$^G$ di sviluppo$^G$
    \par Il Sig. Piccoli ha indicato che non vi sono vincoli particolari riguardo alla scelta del framework$^G$ da utilizzare. Qualsiasi tecnologia è accettabile, sebbene in azienda facciano uso prevalente di Java con Tomcat e PostgreSQL.
    \item API$^G$ disponibili
    \par Saranno messe a disposizione API$^G$ di OpenAI e di soluzioni di intelligenza artificiale open source.
...
\section*{Conclusioni}
Il Sig. Piccoli si è dimostrato collaborativo ed entusiasta per la discussione del progetto$^G$ e ha risposto in modo chiaro alle domande che gli abbiamo posto.

\vspace{1\baselineskip}

Il meeting è stato utile al gruppo per confermare il nostro interesse e le aspettative del capitolato$^G$, e quindi per aiutare nell'analisi dei costi / tempi per la candidatura$^G$ a venire.
