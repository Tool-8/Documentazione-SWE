...
\end{itemize}
\subparagraph{BaseModal} Componente che definisce la struttura scheletrica comune a tutti i dialog dell'applicazione, esponendo tre slot (header, body e footer) che permettono ai componenti di iniettare il contenuto specifico mantenendo una presentazione visiva coerente. Segue la descrizione dei \textit{prop}:
\begin{itemize}
...
\end{itemize}
Segue la descrizione degli \textit{emit}:
\begin{itemize}
...
\subparagraph{NoteArchiveCard} Componente che rappresenta una nota nell'archivio, visualizzando le informazioni principali. Quando cliccata, reindirizza tramite \texttt{<RouterLink>} alla rotta \texttt{/notes/:id} per aprirla nell'editor. Segue la descrizione dei \textit{prop}:
\begin{itemize}
    \item \texttt{string \$id} - id univoco della nota
    \item \texttt{string \$name} - nome della nota
...
\end{itemize}
\subparagraph{useNoteEditorUI} Composable responsabile della gestione dell'interfaccia dell'editor. Si occupa esclusivamente della logica di presentazione, delegando la logica di business a \texttt{useNoteEditor} e le chiamate AI e \texttt{useAi}.
\subparagraph{AsideAI}
\subparagraph{NoteEditorHeader}
...
\subparagraph{NoteEditorContent} Componente che costituisce l'area di lavoro principale dell'editor, diviso in due sezioni, sezione di editor e sezione di render.
