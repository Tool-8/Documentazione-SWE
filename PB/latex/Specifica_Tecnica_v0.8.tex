...
\subparagraph{RouterView} Componente nativo di \texttt{Vue Router} che funge da contenitore dinamico all'interno di \textitt{AppLayout}, responsabile del rendering della pagina corrispondente alla rotta \texttt{URL} attiva. A seconda del percorso di navigazione, il componente monta in tempo reale la pagina appropriata: \texttt{NoteEditorPage} (pagina dell'editor della nota) e \texttt{NoteArchivePage} (pagina dell'archivio delle note). La mappatura è definita in \texttt{index.ts}.:
\begin{itemize}
...
\end{figure}
\subparagraph{NoteArchivePage}
\subparagraph{useContextMenu}
\subparagraph{ContextAction (Type)}
\subparagraph{ContextMenu}
...
\end{figure}
\subparagraph{NoteEditorPage}
\subparagraph{AsideAI}
\subparagraph{NoteEditorHeader}
\subparagraph{NoteEditorActions}
\subparagraph{NoteEditorToolbar}
\subparagraph{NoteEditorContent}
\subparagraph{useNoteEditor}
\subparagraph{useNoteEditorUI}
