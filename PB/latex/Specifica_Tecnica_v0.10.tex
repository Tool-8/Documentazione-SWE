...
\begin{tabular}{|M{0.8cm}|p{2.2cm}|p{5.7cm}|M{1.8cm}|p{2.2cm}|}
  ...
  \hline
  0.10 & Gabriele Disa & Aggiornamento sistema di deploy (sezione 3.2) & 14/05/2026 & - \\
  \hline
...
La persistenza dei dati è gestita tramite un database SQLite salvato in un volume Docker dedicato.

\begin{itemize}
    \item Il database applicativo viene creato automaticamente nel percorso interno:
        \begin{lstlisting}
        /data/database.sqlite
        \end{lstlisting}
    \item Il percorso \texttt{/data} viene associato a un volume Docker persistente;
    \item la cartella \texttt{storage} di Laravel viene anch'essa associata a un volume Docker, in modo da preservare file generati, log e contenuti applicativi salvati su filesystem;
...
\begin{itemize}
    ...
    \item \texttt{db}: PostgreSQL non viene avviato, poiché il deploy finale utilizza SQLite persistente;
