...
\begin{longtable}{|l|p{4cm}|p{8cm}|p{1cm}|}
    ...
    \endfirsthead
    \hline
    \testU & NoteController & Verificare che index() restituisca 200 con la lista delle note & S \\ \hline
    \testU & NoteController & Verificare che index() restituisca 200 con lista vuota quando non ci sono note & S \\ \hline
    \testU & NoteController & Verificare che show(string \$id) restituisca 200 con i dati della nota & S \\ \hline
    \testU & NoteController & Verificare che show(string \$id) restituisca 404 quando la nota non è trovata & S \\ \hline
    \testU & NoteController & Verificare che show(string \$id) restituisca 400 quando l'id è invalido & S \\ \hline
    \testU & NoteController & Verificare che store(Request) restituisca 201 con la nota creata & S \\ \hline
    \testU & NoteController & Verificare che store(Request) utilizzi stringa vuota per content\_md assente & S \\ \hline
    \testU & NoteController & Verificare che store(Request) restituisca 409 quando il titolo è già in uso & S \\ \hline
    \testU & NoteController & Verificare che update(Request, string \$id) restituisca 200 con la nota aggiornata & S \\ \hline
    \testU & NoteController & Verificare che update(Request, string \$id) restituisca 404 quando la nota non è trovata & S \\ \hline
    \testU & NoteController & Verificare che update(Request, string \$id) restituisca 409 quando il titolo è già in uso & S \\ \hline
    \testU & NoteController & Verificare che update(Request, string \$id) restituisca 400 quando l'id è invalido & S \\ \hline
    \testU & NoteController & Verificare che destroy(string \$id) restituisca 204 in caso di successo & S \\ \hline
    \testU & NoteController & Verificare che destroy(string \$id) restituisca 404 quando la nota non è trovata & S \\ \hline
    \testU & NoteController & Verificare che destroy(string \$id) restituisca 500 quando l'eliminazione fallisce & S \\ \hline
    \testU & NoteController & Verificare che destroy(string \$id) restituisca 400 quando l'id è invalido & S \\ \hline
    \testU & NoteController & Verificare che un'eccezione RuntimeException sconosciuta restituisca 500 & S \\ \hline
    \testU & NoteService & Verificare che list():array restituisca correttamente la lista completa delle note nell'archivio ottenuta da NoteRepositoryInterface & S \\ \hline
    \testU & NoteService & Verificare che list():array restituisca una lista vuota quando non ci sono note nell'archivio & S \\ \hline
    \testU & NoteService & Verificare che get(string \$id):array restituisca i dettagli della nota corrispondente all'ID specificato & S \\ \hline
    \testU & NoteService & Verificare che get(string \$id):array restituisca un errore in caso di nota non esistente per l'id specificato & S \\ \hline
    \testU & NoteService & Verificare che create(string \$title, string \$contentMd):array deleghi a NoteRepositoryInterface la creazione di una nuova nota nell'archivio (e restituisca un valore di successo) & S \\ \hline
    \testU & NoteService & Verificare che create(string \$title, string \$contentMd):array restituisca un errore in caso la creazione di una nuova nota non sia avvenuta qualora fosse già presente una nota con lo stesso nome & S \\ \hline
    \testU & NoteService & Verificare che update(string \$id, string \$title, string \$contentMd):array deleghi a NoteRepositoryInterface l'aggiornamento dei dettagli di una nota esistente (e ritorni un valore di successo) & S \\ \hline
    \testU & NoteService & Verificare che update(string \$id, string \$title, string \$contentMd):array restituisca un errore e non deleghi l'aggiornamento dei dettagli della nota scelta qualora fosse già presente una nota con lo stesso nome & S \\ \hline
    \testU & NoteService & Verificare che delete(string \$id):void deleghi correttamente l'eliminazione della nota corrispondente all'ID specificato a NoteRepositoryInterface (e ritorni un valore di successo) & S \\ \hline
    \testU & NoteRepository & Verificare che list():array restituisca correttamente la lista completa delle note nell'archivio & S \\ \hline
    \testU & NoteRepository & Verificare che list():array restituisca una lista vuota quando non ci sono note nell'archivio & S \\ \hline
    \testU & NoteRepository & Verificare che get(string \$id):array restituisca i dati completi della nota corrispondente all'ID specificato & S \\ \hline
    \testU & NoteRepository & Verificare che venga gestita l'eccezione quando si tenta di utilizzare get(string \$id):array con un ID di una nota non esistente & S \\ \hline
    \testU & NoteRepository & Verificare che create(string \$title, string \$contentMd):array crei correttamente una nuova nota nell'archivio, associandola ad un ID univoco & S \\ \hline
    \testU & NoteRepository & Verificare che update(string \$id, string \$title, string \$contentMd):array aggiorni correttamente il titolo della nota scelta & S \\ \hline
    \testU & NoteRepository & Verificare che update(string \$id, string \$title, string \$contentMd):array aggiorni correttamente il contenuto della nota scelta & S \\ \hline
    \testU & NoteRepository & Verificare che delete(string \$id):void elimini correttamente la nota corrispondente all'ID specificato & S \\ \hline
    \testU & NoteRepository & Verificare che delete(string \$id):void gestisca correttamente l'eliminazione di una nota non esistente & S \\ \hline
    \testU & NoteRepository & Verificare che isTitleUsed(string \$title, ?string \$excludeId = null):bool restituisca true se un titolo è già utilizzato da un'altra nota con ID diverso da excludeId & S \\ \hline
    \testU & NoteRepository & Verificare che isTitleUsed(string \$title, ?string \$excludeId = null):bool restituisca false se un titolo non è già utilizzato da un'altra nota con ID diverso da excludeId & S \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca la nota corrispondente all'ID specificato nel formato richiesto qualora esso sia valido & - \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 404 qualora l'ID non sia valido & - \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 404 qualora il formato richiesto non sia valido & - \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 404 in caso di errore durante l'esportazione & - \\ \hline
    \testU & ExportService & Verificare che export(string \$id, string \$format):array restituisca correttamente i dati della nota, nel formato corretto dopo averne delegato l'esportazione a ExportStrategyInterface & - \\ \hline
    \testU & ExportStrategyFactory & Verificare che make(string \$format):ExportStrategyInterface istanzi e restituisca la strategy concreta MdExport nel caso il formato richiesto sia "md" & - \\ \hline
    \testU & ExportStrategyFactory & Verificare che make(string \$format):ExportStrategyInterface istanzi e restituisca la strategy concreta HtmlExport nel caso il formato richiesto sia "html" & - \\ \hline
    \testU & ExportStrategyFactory & Verificare che make(string \$format):ExportStrategyInterface istanzi e restituisca la strategy concreta PdfExport nel caso il formato richiesto sia "pdf" & - \\ \hline
    \testU & MdExport & Verificare che export(string \$content, string \$title):string converta correttamente il contenuto della nota in formato Markdown & - \\ \hline
    \testU & HtmlExport & Verificare che export(string \$content, string \$title):string converta correttamente il contenuto della nota in formato HTML & - \\ \hline
    \testU & PdfExport & Verificare che export(string \$content, string \$title):string converta correttamente il contenuto della nota in formato PDF & - \\ \hline
    \testU & MdExport & Verificare che contentType():string restituisca il MIME type corrispondente al formato Markdown & - \\ \hline
    \testU & HtmlExport & Verificare che contentType():string restituisca il MIME type corrispondente al formato HTML & - \\ \hline
    \testU & PdfExport & Verificare che contentType():string restituisca il MIME type corrispondente al formato PDF & - \\ \hline
    \testU & MdExport & Verificare che extension():string restituisca l'estensione del file corrispondente al formato Markdown & - \\ \hline
    \testU & HtmlExport & Verificare che extension():string restituisca l'estensione del file corrispondente al formato HTML & - \\ \hline
    \testU & PdfExport & Verificare che extension():string restituisca l'estensione del file corrispondente al formato PDF & - \\ \hline
    \testU & LlmController & Verificare che invoke(Request \$request, string \$id):JsonResponse deleghi correttamente a LlmService l'elaborazione della richiesta restituendo il risultato in una risposta JSON con status 200 & - \\ \hline
    \testU & LlmController & Verificare che invoke(Request \$request, string \$id):JsonResponse non deleghi a LlmService l'elaborazione della richiesta, qualora i parametri non siano validi, e restituisca un errore & - \\ \hline
    \testU & LlmController & - & - \\ \hline
    \testU & LlmService & Verificare che process(string \$content, string \$action, array \$options = []):string costruisca correttamente un oggetto LlmContext & - \\ \hline
    \testU & LlmService & Verificare che process(string \$content, string \$action, array \$options = []):string costruisca correttamente un oggetto LlmContext con l'azione richiesta & - \\ \hline
    \testU & LlmService & Verificare che process(string \$content, string \$action, array \$options = []):string costruisca correttamente un oggetto LlmContext con il testo da elaborare corretto & - \\ \hline
    \testU & LlmService & Verificare che process(string \$content, string \$action, array \$options = []):string costruisca correttamente un oggetto LlmContext con le opzioni specificate & - \\ \hline
    \testU & LlmService & Verificare che process(string \$content, string \$action, array \$options = []):string restituisca il testo prodotto da LlmStrategyInterface & - \\ \hline
    \testU & LllmService & - & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta Summarize nel caso l'azione richiesta sia di riassunto & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta Translate nel caso l'azione richiesta sia di traduzione & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta Rewrite nel caso l'azione richiesta sia di riscrittura & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta DistantWriting nel caso l'azione richiesta sia di generazione di testo & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta WhiteHat nel caso l'azione richiesta sia di critica del testo secondo cappello bianco & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta RedHat nel caso l'azione richiesta sia di critica del testo secondo cappello rosso & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta BlackHat nel caso l'azione richiesta sia di critica del testo secondo cappello nero & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta YellowHat nel caso l'azione richiesta sia di critica del testo secondo cappello giallo & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta GreenHat nel caso l'azione richiesta sia di critica del testo secondo cappello verde & - \\ \hline
    \testU & LlmStrategyFactory & Verificare che make(string \$action):LlmStrategyInterface istanzi e restituisca la strategy concreta BlueHat nel caso l'azione richiesta sia di critica del testo secondo cappello blu & - \\ \hline
    \testU & LlmProcessor & Verificare che setStrategy(LlmStrategyInterface \$strategy):static imposti correttamente la strategy da utilizzare & - \\ \hline
    \testU & LlmProcessor & Verificare che run(LlmContext \$context):string deleghi correttamente l'elaborazione del testo alla strategy impostata & - \\ \hline
    \testU & LlmProcessor & Verificare che run(LlmContext \$context):string restituisca il testo prodotto dalla strategy impostata & - \\ \hline
    \testU & Summarize & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di riassunto & - \\ \hline
    \testU & Summarize & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & - & - & - \\ \hline
    \testU & Translate & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di traduzione & - \\ \hline
    \testU & Translate & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & Rewrite & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di riscrittura & - \\ \hline
    \testU & Rewrite & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & DistantWriting & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di generazione di testo & - \\ \hline
    \testU & DistantWriting & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & WhiteHat & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di critica del testo secondo cappello bianco & - \\ \hline
    \testU & WhiteHat & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & RedHat & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di critica del testo secondo cappello rosso & - \\ \hline
    \testU & RedHat & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & BlackHat & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di critica del testo secondo cappello nero & - \\ \hline
    \testU & BlackHat & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & YellowHat & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di critica del testo secondo cappello giallo & - \\ \hline
    \testU & YellowHat & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & GreenHat & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di critica del testo secondo cappello verde & - \\ \hline
    \testU & GreenHat & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
    \testU & BlueHat & Verificare che process(LlmContext \$context):string crei il prompt corretto per la richiesta di critica del testo secondo cappello blu & - \\ \hline
    \testU & BlueHat & Verificare che process(LlmContext \$context):string restituisca il testo prodotto dalla LLM & - \\ \hline
\end{longtable}
...
\begin{longtable}{|l|p{8cm}|p{2cm}|p{1.5cm}|}
    ...
    \testS & Verificare che il sistema consenta la selezione esclusivamente all'interno delle porzioni di testo tradotte, impedendo la selezione di altre parti del testo durante tale operazione & RF-O-77 & - \\ \hline
    \testS & Verificare che il sistema consenta la selezione esclusivamente all'interno delle porzioni di testo originale (associato ad una traduzione), impedendo la selezione di altre parti del testo durante tale operazione & RF-O-78 & - \\ \hline
    \testS & Verificare che l'agente produca traduzioni corrette nella lingua selezionata & RF-O-79 & - \\ \hline
    \testS & Verificare che il sistema, alla chiusura di una nota, se presenti delle modifiche non salvate avvisi l'utente della presenza di modifiche & RF-O-80 & - \\ \hline
    \testS & Verificare che il sistema in caso di problemi di connessione con l'agente esterno avvisi l'utente & RF-O-81 & -  \\ \hline
    \testS & Verificare che, durante una richiesta alla LLM, il sistema fornisca all’utente un feedback chiaro sullo stato dell’operazione (es. indicatore di caricamento/elaborazione) fino al completamento o alla segnalazione di errore & RF-O-82 & - \\ \hline
    \testS & Verificare che il feedback sullo stato della comunicazione con la LLM sia non bloccante ed eviti che l’utente avvii richieste duplicate inconsapevolmente & RF-O-83 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di visualizzare l'editor con il contenuto della nota dopo l'apertura di questa & RF-O-84 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di tentativo di apertura di una nota non più esistente & RF-O-85 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di fallimento dell'apertura di una nota a causa di problemi di connessione al server & RF-O-86 & - \\ \hline
    \testS & Verificare che, in caso di tentativo di chiusura di una nota con modifiche non salvate, il sistema avvisi l'utente e chieda se desidera salvare le modifiche prima di chiudere & RF-O-87 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di salvare le modifiche effettuate all'interno di una nota & RF-O-88 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di presenza di una versione più recente della nota aperta nell'archivio & RF-O-89 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di sovrascrivere la nota presente nell'archivio con la versione aperta in caso di conflitto di versioni & RF-O-90 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di salvare la nota aperta con un nuovo nome in caso di conflitto con la versione presente nell'archivio & RF-O-91 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di aggiornare la nota aperta con le modifiche presenti nella versione più recente nell'archivio in caso di conflitto di versioni & RF-O-92 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di errori durante il salvataggio della nota causati da problemi di connessione al server & RF-O-93 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di tentativo di clonazione di una nota non più esistente & RF-O-94 & - \\ \hline
    \testS & Verificare che il sistema avvisi in caso di impossibilità di clonare una nota a causa di problemi di connessione al server & RF-O-95 & - \\ \hline
    \testS & Verificare che il sistema chieda all'utente di confermare l'eliminazione di una nota & RF-O-96 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di impossibilità di eliminazione di una nota a causa di problemi di connessione al server & RF-O-97 & - \\ \hline
    \testS & Verificare che il sistema impedisca all'utente di rinominare una nota con un nome già presente nell'archivio e visualizzare un messaggio di errore & RF-O-98 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di impossibilità di rinominare una nota a causa di problemi di connessione al server & RF-O-99 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di impossibilità di esportare una nota a causa di problemi di connessione al server & RF-O-100 & - \\ \hline
    \testS & Verificare che il sistema dia la possibilità all'utente di importare una nota in formato MD & RF-O-101 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di problemi di connessione al server durante l'importazione di una nota & RF-O-102 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di inserire elementi in un elenco puntato o numerato & RF-O-103 & - \\ \hline
    \testS & Verificare che il sistema permetta all'utente di rimuovere elementi da un elenco puntato o numerato & RF-O-104 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di problemi di connessione con l'agente esterno durante la ritraduzione di una porzione di testo precedentemente tradotta & RF-O-105 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di mancata registrazione/autenticazione dovuta a problemi di connessione al server & RF-O-106 & - \\ \hline
    \testS & Verificare che il sistema avvisi l'utente in caso di impossibilità di eliminazione dell'account a causa di problemi di connessione al server & RF-O-107 & - \\ \hline
    \testS & Verificare che il sistema impedisca la selezione contemporanea di porzioni di testo appartenenti a una traduzione (testo originale o testo tradotto) e porzioni di testo non appartenenti ad alcuna traduzione & RF-O-108 & - \\ \hline
...
\end{figure}
Dal grafico si osserva l'andamento dell'EAC nei sette sprint. Nel terzo periodo il valore si mantiene al di sopra del BAC, indicando una tendenza al superamento del budget pianificato; tuttavia, la traiettoria mostra un progressivo avvicinamento alla linea di base, suggerendo un miglioramento nella gestione delle risorse. Nel settimo periodo invece si nota uno scostamento del valore EAC oltre il limite inferiore, dovuto ad un aumento dell'efficienza da parte del gruppo. È necessario continuare a ottimizzare le stime per rientrare entro i limiti accettabili (±5\% BAC) nei periodi successivi.
...
\end{figure}
Il grafico mette in relazione AC, ETC, BAC e la loro somma AC+ETC nel corso dei sette sprint. La linea AC+ETC, che rappresenta la proiezione del costo finale, si mantiene al di sopra del BAC per quasi tutti i periodi, confermando un rischio di sforamento del budget stimabile intorno al 3--4\%. Tuttavia, nel quarto sprint si osserva un avvicinamento della linea AC+ETC al BAC, segnale positivo di un progressivo rientro nei costi pianificati. Dal quinto sprint in poi, la linea AC+ETC si mantiene prossima al valore desiderato (BAC). Nel settimo sprint inoltre si osserva una diminuzione della proiezione AC+ETC, risultato di un aumento della produttività del gruppo. È opportuno mantenere questo trend nei periodi conclusivi per minimizzare lo scarto finale.
