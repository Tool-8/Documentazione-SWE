...
\usepackage{forest}
\usepackage{xcolor}
\definecolor{commentgray}{RGB}{140,140,140}
...
\begin{tabular}{|M{0.8cm}|p{2.2cm}|p{5.7cm}|M{1.8cm}|p{2.2cm}|}
  ...
  \hline
  1.0 & spadiliero ruben & approvazione documento per pb & 14/05/206 & Gabriele Disa \\
   \hline
...
\begin{itemize}
  ...
  \item \href{https://tool-8.github.io/Documentazione-SWE/pb/Analisi_dei_Requisiti_v2.0.pdf}{Analisi dei Requisiti v2.0}
\end{itemize}
...
Il \textit{frontend} dell'applicazione è sviluppato con il framework Vue.js, adottando nativamente il pattern architetturale \textit{MVVM} (Model-View-ViewModel), che separa le responsabilità tra logica di business, stato dell'interfaccia e presentazione visiva. In particolare, il ViewModel è implementato tramite la \textit{Composition API}, che gestisce lo stato locale del componente, reagisce ai cambiamenti del Model e aggiorna la View in modo reattivo. La struttura delle cartelle è la seguente:


\forestset{
  comment/.style={text=commentgray, font=\ttfamily\small\itshape}
}
...
[ts/
  ...
  [services/
    [{\# File per chiamate API al backend attraverso la libreria Axios, comment}]
  ]
  [tests/
    [{\# Test unitari del frontend, comment}]
  ]
  [types/
...
Il deployment finale viene realizzato tramite un'\textbf{immagine Docker monolitica}, contenente al proprio interno tutti i componenti necessari all'esecuzione dell'applicazione: server web, runtime PHP, codice applicativo Laravel, dipendenze backend, build degli asset frontend e worker delle code. In questo modo l'applicazione può essere avviata in modo riproducibile su qualunque ambiente che supporti Docker, senza richiedere l'installazione manuale di Composer, Node.js, npm o altre dipendenze applicative sulla macchina ospitante.

\vspace{1em}

Docker Compose viene utilizzato come strumento di supporto per semplificare l'avvio del container, la configurazione delle variabili d'ambiente, il mapping delle porte e la gestione dei volumi persistenti. Nel deploy finale non viene però adottata una suddivisione in più container: Docker Compose avvia un unico servizio applicativo, mantenendo il sistema semplice da distribuire anche su server di produzione.

\subsubsection{Organizzazione del container}
Nel sistema di deploy finale, l'applicazione è eseguita all'interno di un unico container denominato \texttt{app}. Tale container racchiude le responsabilità tecniche necessarie all'erogazione del servizio, pur mantenendo l'architettura applicativa monolitica.

\begin{itemize}
    \item \texttt{app}
        \begin{itemize}
            \item Contiene l'applicazione Laravel;
            \item contiene le dipendenze PHP installate tramite Composer durante la fase di build dell'immagine;
            \item contiene gli asset frontend compilati tramite Vite durante la fase di build dell'immagine;
            \item utilizza Nginx come web server e punto di ingresso per le richieste HTTP;
            \item utilizza PHP-FPM per l'esecuzione del codice PHP dell'applicazione;
            \item esegue il worker delle code Laravel;
            \item utilizza Supervisor per gestire i processi interni al container;
            \item espone l'applicazione sulla porta interna \texttt{8080}, mappabile su una porta esterna configurabile;
            \item utilizza SQLite come sistema di persistenza dei dati applicativi.
        \end{itemize}
\end{itemize}

\subsubsection{Persistenza dei dati}
La persistenza dei dati è gestita tramite volumi Docker dedicati. Nel progetto attuale le note vengono salvate come file testuali nello storage locale di Laravel, mentre il database SQLite viene utilizzato solo per le tabelle infrastrutturali previste dalle migration Laravel, come sessioni, cache e code.

\begin{itemize}
    \item Le note e gli altri file applicativi vengono preservati tramite un volume Docker associato alla cartella:
        \begin{lstlisting}
        /var/www/html/storage
        \end{lstlisting}

    \item Il database SQLite viene creato automaticamente nel percorso interno:
        \begin{lstlisting}
        /data/database.sqlite
        \end{lstlisting}

    \item Il percorso \texttt{/data} viene associato a un volume Docker persistente;

    \item Il database SQLite non contiene le note, ma dati infrastrutturali Laravel, come sessioni, cache e code;

    \item Le migration Laravel vengono eseguite automaticamente all'avvio del container.
\end{itemize}


\subsubsection{Build degli asset frontend}
Gli asset frontend non vengono compilati in fase di esecuzione del container. Durante la build dell'immagine Docker viene eseguita una fase dedicata alla compilazione del frontend tramite Vite.

\begin{itemize}
    \item Le dipendenze JavaScript vengono installate durante la build;
    \item Vite produce gli asset statici ottimizzati per l'ambiente di produzione;
    \item gli asset generati vengono copiati nell'immagine finale e serviti da Nginx;
    \item non è necessario eseguire un dev server Vite nell'ambiente di produzione.
\end{itemize}

Il container \texttt{vite} rimane quindi utile esclusivamente in contesti di sviluppo, mentre non fa parte dell'architettura di deploy finale.

\subsubsection{Configurazione e avvio}
La configurazione del deploy avviene tramite variabili d'ambiente, definite nel file \texttt{.env} posto nella root del progetto o direttamente nell'ambiente di esecuzione del container.
...
Tale comando costruisce l'immagine Docker monolitica e avvia il container tramite il file \texttt{docker-compose.deploy.yml}. Il sistema di avvio si occupa automaticamente di:

\begin{itemize}
    \item creare il file di configurazione Laravel all'interno del container;
    \item generare una chiave applicativa, se non già configurata;
    \item creare il database SQLite, se non presente;
    \item eseguire le migration;
    \item preparare le cache applicative;
    \item avviare Nginx, PHP-FPM e il worker delle code Laravel.
\end{itemize}

La porta esterna può essere configurata tramite la variabile \texttt{APP\_PORT}; internamente il container espone il servizio sulla porta \texttt{8080}. Ad esempio, impostando:
...
l'applicazione sarà accessibile sulla porta \texttt{8090}, mentre il servizio interno continuerà a utilizzare la porta \texttt{8080}.

\subsubsection{Container non presenti nel deploy finale}
Rispetto all'ambiente di sviluppo o a precedenti configurazioni, il deploy finale non prevede i seguenti container separati:

\begin{itemize}
    \item \texttt{web}: le funzionalità di web server sono incluse nel container \texttt{app} tramite Nginx;
    \item \texttt{db}: PostgreSQL non viene avviato;
    \item \texttt{vite}: la compilazione degli asset frontend avviene durante la build dell'immagine Docker e non tramite un dev server in produzione.
\end{itemize}

Questa organizzazione consente di ottenere un deploy più semplice e portabile, mantenendo un unico container applicativo e riducendo le dipendenze operative richieste all'ambiente ospitante.
...
\section{Stato dei requisiti funzionali}
    ...
    \begin{longtable}{|l|p{8cm}|p{3.5cm}|}
            ...
            \endfirsthead
            \hline
            \reqF{O} & Il sistema deve permettere all'utente di visualizzare l'archivio delle note presenti e per ogni nota il suo nome, la data di creazione e la data dell'ultima modifica & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di aprire una nota & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di uscire da una nota & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di creare una nuova nota & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di salvare la nota su cui sta lavorando & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di salvare una nota non ancora presente nell'archivio per la prima volta & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di clonare una nota & SI \\ \hline
            \reqF{O} & Alla clonazione di una nota, il sistema deve assegnare automaticamente un nome di default (es. "Nota \textit{data-ora}") che l’utente può modificare & SI \\ \hline
            \reqF{O} & Il nome di default generato nella clonazione di una nota deve essere univoco all'interno dell'archivio e facilmente riconoscibile all'utente & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di ricercare le note per nome nell'archivio & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di eliminare una nota & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di rinominare una nota & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di esportare una nota aperta & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di esportare una nota direttamente dall'archivio & SI \\ \hline
            \reqF{O} & L’esportazione deve preservare integralmente il contenuto testuale della nota, senza perdita o alterazione di caratteri & SI \\ \hline
            \reqF{O} & L’esportazione deve preservare correttamente i caratteri Unicode del testo (es. accenti e simboli), senza sostituzioni o corruzione & SI \\ \hline
            \reqF{O} & Al termine dell’esportazione, il sistema deve notificare esplicitamente l’errore di esportazione all'utente & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di importare una nota & SI \\ \hline
            \reqF{O} & L’importazione deve preservare integralmente il contenuto testuale della nota, senza perdita o alterazione di caratteri (incl. Unicode) & SI \\ \hline
            \reqF{O} & In caso di file non valido o corrotto, il sistema deve interrompere l’importazione senza creare note parziali e deve notificare chiaramente l’errore all’utente & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all’utente di scrivere nell'editor con caratteri Unicode (es. lettere accentate e simboli) & SI \\ \hline
            \reqF{O} & Il sistema deve compilare la sintassi Markdown  & SI \\ \hline
            \reqF{O} & Il sistema deve permettere la visualizzazione del documento formattato secondo la sintassi Markdown &  SI \\ \hline
            \reqF{O} & L'aggiornamento della visualizzazione formattata deve avvenire in un tempo adeguatamente veloce & SI \\ \hline
            \reqF{O} & Il sistema deve permettere di cambiare modalità di visualizzazione & SI \\ \hline
            \reqF{O} & Il sistema deve fornire un'area di editing di testo in cui è possibile inserire e modificare del testo per la compilazione Markdown & SI \\ \hline
            \reqF{O} & Il sistema deve permettere l'applicazione di formattazioni Markdown senza scrivere manualmente la sintassi & SI \\ \hline
            \reqF{O} & L’editor deve rispondere all’input e alla formattazione dell’utente in modo fluido, senza ritardi percepibili durante la digitazione & SI \\ \hline
            \reqF{O} & Il sistema deve permettere l'applicazione e la rimozione della formattazione testo grassetto senza scrivere manualmente la sintassi & SI \\ \hline
            \reqF{O} & Il sistema deve permettere l'applicazione e la rimozione della formattazione testo corsivo senza scrivere manualmente la sintassi & SI \\ \hline
            \reqF{O} & Il sistema deve permettere l'applicazione e la rimozione della formattazione testo sottolineato senza scrivere manualmente la sintassi & SI \\ \hline
            \reqF{O} & Il sistema deve consentire l’applicazione e la rimozione della formattazione testo barrato senza richiedere l’inserimento manuale della sintassi & SI \\ \hline
            \reqF{O} & Il sistema deve consentire l'inserimento e la rimozione di link ipertestuali senza richiedere l'inserimento manuale della sintassi & SI \\ \hline
            \reqF{O} & L’editor deve rendere i link ipertestuali facilmente riconoscibili (es. evidenziandoli graficamente) e distinguibili dal testo normale & SI \\ \hline
            \reqF{O} & Il sistema deve consentire l'applicazione e la rimozione della formattazione testo commentato senza richiedere l'inserimento manuale della sintassi & SI \\ \hline
            \reqF{O} & Il sistema deve consentire l'inserimento di elenchi numerati & SI \\ \hline
            \reqF{O} & Il sistema deve consentire l'inserimento di elenchi puntati & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di annullare l'ultima modifica fatta & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di ripristinare la modifica precedentemente annullata & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di selezionare porzioni di testo, sia tramite mouse che tastiera & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di copiare il testo selezionato & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di tagliare il testo selezionato & SI \\ \hline
            \reqF{O} & L'editor deve poter fare copiare e tagliare all'utente il testo in modo fluido, senza ritardi percepibili & SI \\ \hline
            \reqF{O} & L'editor deve preservare correttamente il contenuto e la struttura del testo copiato e tagliato, senza perdita o alterazioni di caratteri & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di incollare del testo nell'area di testo & SI \\ \hline
            \reqF{O} & L'editor deve poter fare incollare all'utente il testo in modo fluido, senza ritardi percepibili & SI \\ \hline
            \reqF{O} & L'editor deve preservare e incollare correttamente il contenuto e la struttura del testo, senza perdita o alterazioni di caratteri & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di esportare una nota aperta in MD & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di esportare una nota direttamente dall'archivio in MD & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente della presenza di una nota con lo stesso nome in seguito al primo salvataggio & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo secondo il modello dei "Sei Cappelli" & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo con una "Visione Emotiva" (Cappello Rosso) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo con una "Visione Neutra" (Cappello Bianco) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo con una "Visione Ottimista" (Cappello Giallo) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo con una "Visione dell'avvocato del Diavolo" (Cappello Nero) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo con una "Visione Originale" (Cappello Verde) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di criticare il testo con una "Visione Logica e Strutturata" (Cappello Blu) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di riassumere del testo tramite un agente esterno & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di riscrivere del testo tramite un agente esterno & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di scegliere il modo in cui verrà riscritto il testo (correzione grammaticale, estensione del testo, miglioramento del lessico, variazione stilistica) & SI   \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di tradurre del testo tramite un agente esterno & SI \\ \hline
            \reqF{O} & Il sistema deve permettere l'invio da parte dell'utente di un prompt a un agente esterno per la generazione di testo (Distant Writing) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di sostituire nell'editor il testo originale con quello generate dall'agente esterno (riassunto, riscrittura, traduzione) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di aggiungere nell'editor, in seguito alla selezione del testo originale, quello generato dall'agente esterno (riassunto, riscrittura, traduzione) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di aggiungere nell'editor, in coda a tutto il testo presente nell'editor, quello generato dall'agente esterno (riassunto, riscrittura, traduzione) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di "rifiutare" il testo generato dall'agente esterno (riassunto, riscrittura, traduzione) & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di copiare negli appunti il testo generato dall'agente esterno (riassunti, riscrittura, traduzione, generazione) & SI \\ \hline
            \reqF{O} & L'agente esterno deve produrre testo grammaticalmente, sintatticamente e semanticamente corretto & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso dell'impossibilità nell'eseguire una richiesta & SI  \\ \hline
            \reqF{O} & Il sistema deve commentare il testo selezionato dall’utente utilizzato per interrogare la generazione (Distant Writing) & SI \\ \hline
            \reqF{O} & Se l’utente aggiunge una traduzione a una parte del testo selezionato, il sistema deve avvisarlo ogni volta che il testo tradotto viene modificato & SI \\ \hline
            \reqF{O} & Il sistema deve consentire all’utente di aggiornare una traduzione precedentemente inserita se il testo originale è presente ed è stato modificato & SI \\ \hline
            \reqF{O} & Il sistema deve consentire all'utente di rimuovere una traduzione precedentemente inserita & SI \\ \hline
            \reqF{O} & Il sistema deve consentire all'utente di rimuovere il testo originale di una traduzione precedentemente inserita & SI \\ \hline
            \reqF{O} & Il sistema deve notificare all'utente la perdita del testo originale di riferimento qualora venga eliminato, lasciando la traduzione priva di collegamento & SI \\ \hline
            \reqF{O} & Il sistema non deve consentire l'inserimento di una traduzione all'interno di una porzione di testo già tradotta & SI \\ \hline
            \reqF{O} & L'agente deve produrre traduzioni corrette nella lingua selezionata & SI \\ \hline
            \reqF{O} & Il sistema alla chiusura di una nota, se presenti delle modifiche non salvate deve avvisare l'utente della presenza di modifiche & SI \\ \hline
            \reqF{O} & Il sistema in caso di problemi di connessione con l'agente esterno deve avvisare l'utente & SI  \\ \hline
            \reqF{O} & Durante una richiesta alla LLM, il sistema deve fornire all’utente un feedback chiaro sullo stato dell’operazione (es. indicatore di caricamento/elaborazione) fino al completamento o alla segnalazione di errore & SI \\ \hline
            \reqF{O} & Il feedback sullo stato della comunicazione con la LLM deve essere non bloccante e deve evitare che l’utente avvii richieste duplicate inconsapevolmente & SI\\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di visualizzare l'editor con il contenuto della nota dopo l'apertura di questa & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di tentativo di apertura di una nota non più esistente & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di fallimento dell'apertura di una nota a causa di problemi di connessione al server & SI \\ \hline
            \reqF{O} & In caso di tentativo di chiusura di una nota con modifiche non salvate, il sistema deve avvisare l'utente e chiedere se desidera salvare le modifiche prima di chiudere & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di salvare le modifiche effettuate all'interno di una nota & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di presenza di una versione più recente della nota aperta nell'archivio & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di sovrascrivere la nota presente nell'archivio con la versione aperta in caso di conflitto di versioni & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di salvare la nota aperta con un nuovo nome in caso di conflitto con la versione presente nell'archivio & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di aggiornare la nota aperta con le modifiche presenti nella versione più recente nell'archivio in caso di conflitto di versioni & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di errori durante il salvataggio della nota causati da problemi di connessione al server & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di tentativo di clonazione di una nota non più esistente & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare in caso di impossibilità di clonare una nota a causa di problemi di connessione al server & SI \\ \hline
            \reqF{O} & Il sistema deve chiedere all'utente di confermare l'eliminazione di una nota & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di impossibilità di eliminazione di una nota a causa di problemi di connessione al server & SI \\ \hline
            \reqF{O} & Il sistema deve impedire all'utente di rinominare una nota con un nome già presente nell'archivio e visualizzare un messaggio di errore & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di impossibilità di rinominare una nota a causa di problemi di connessione al server & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di impossibilità di esportare una nota a causa di problemi di connessione al server & SI \\ \hline
            \reqF{O} & Il sistema deve dare la possibilità all'utente di importare una nota in formato MD & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di problemi di connessione al server durante l'importazione di una nota & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di inserire elementi in un elenco puntato o numerato & SI \\ \hline
            \reqF{O} & Il sistema deve permettere all'utente di rimuovere elementi da un elenco puntato o numerato & SI \\ \hline
            \reqF{O} & Il sistema deve avvisare l'utente in caso di problemi di connessione con l'agente esterno durante la ritraduzione di una porzione di testo precedentemente tradotta & SI \\ \hline
            \reqF{D} & Il sistema deve aggiornare automaticamente i risultati della ricerca ad ogni modifica dell’input di ricerca & SI \\ \hline
            \reqF{D} & Il sistema deve permettere all'utente di esportare una nota aperta in PDF & SI \\ \hline
            \reqF{D} & Il sistema deve permettere all'utente di esportare una nota aperta in HTML & SI \\ \hline
            \reqF{D} & Il sistema deve permettere all'utente di esportare una nota direttamente dall'archivio in PDF & SI \\ \hline
            \reqF{D} & Il sistema deve permettere all'utente di esportare una nota direttamente dall'archivio in HTML & SI \\ \hline
            ...
            \reqF{D} & L'archivio deve aggiornarsi dinamicamente con i risultati della ricerca ad ogni modifica dell’input con una risposta percepita come immediata dall’utente &  SI \\ \hline
            \reqF{D} & L'editor deve evidenziare in modo chiaro e distinguibile la porzione di testo utilizzato come prompt per l'interrogazione del LLM, così da renderla immediatamente riconoscibile all'utente & SI \\ \hline
            \reqF{D} & L’aggiornamento della visualizzazione formattata non deve bloccare l’input dell’utente durante la digitazione & SI \\ \hline
            \reqF{OP} & Il sistema deve aggiornare automaticamente la visualizzazione del testo formattato ad ogni modifica del Markdown, senza richiedere un’azione esplicita & SI \\ \hline
