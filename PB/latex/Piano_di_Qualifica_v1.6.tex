...
\begin{tabular}{|M{0.8cm}|p{3cm}|p{4.1cm}|M{1.8cm}|p{3cm}|}
  ...
  \hline
  1.6 & Stefano Maso & Aggiunte metriche per il processo di verifica, aggiunti test di integrazione, aggiunte sezioni cruscotto & 03/05/2026 & - \\
  \hline
  1.5 & Stefano Maso & Aggiornamento grafici del cruscotto con Sprint 10 e aggiunta Test di Unità & 29/04/2026 & - \\
  \hline
...
\begin{longtable}{|l|p{4cm}|p{8cm}|p{1cm}|}
    ...
    \testU & LlmController & Verificare che invoke(Request \$request):JsonResponse deleghi correttamente a LlmService l'elaborazione di un'azione semplice senza opzioni e restituisca il risultato in una risposta JSON con status 200 - verificato da \texttt{LlmControllerTest.php} & S \\ \hline
    \testU & LlmController & Verificare che invoke(Request \$request):JsonResponse deleghi correttamente a LlmService l'elaborazione di un'azione di traduzione con opzioni e restituisca il risultato in una risposta JSON con status 200 - verificato da \texttt{LlmControllerTest.php} & S \\ \hline
    \testU & LlmController & Verificare che invoke(Request \$request):JsonResponse deleghi correttamente a LlmService l'elaborazione di un'azione di riscrittura con opzioni e restituisca il risultato in una risposta JSON con status 200 - verificato da \texttt{LlmControllerTest.php} & S \\ \hline
    \testU & LlmController & Verificare che invoke(Request \$request):JsonResponse lanci una ValidationException qualora l'azione richiesta non sia valida - verificato da \texttt{LlmControllerTest.php} & S \\ \hline
    \testU & LlmController & Verificare che invoke(Request \$request):JsonResponse restituisca una risposta JSON con status 502 qualora si verifichi un errore durante l'elaborazione da parte della LLM - verificato da \texttt{LlmControllerTest.php} & S \\ \hline
...
\begin{longtable}{|l|p{8cm}|p{2cm}|p{1.5cm}|}
    ...
    \testI & Verificare che GET /api/notes/export/\{id\}?format=mdma restituisca 400 con messaggio "Unknown format: mdma" quando viene richiesto un formato non valido con id - verificato da \texttt{ExportTest.php} & S \\ \hline
    \testI & Verificare che POST /api/notes/export restituisca 400 con messaggio "Unknown format: mdma" quando viene richiesto un formato non valido senza id - verificato da \texttt{ExportTest.php} & S \\ \hline
    \testI & Verificare che GET /api/notes/export/\{id\}?format=html restituisca 404 con messaggio "Note not found" quando la nota non esiste - verificato da \texttt{ExportTest.php} & S \\ \hline
    \testI & Verificare che GET /api/notes/export/123?format=html restituisca 400 con messaggio "Invalid note id" quando l'id fornito non è un UUID valido - verificato da \texttt{ExportTest.php} & S \\ \hline
    \testI & Verificare che GET /api/notes/export/\{id\}?format=html restituisca 500 con messaggio "Server error" quando il servizio lancia una RuntimeException inattesa - verificato da \texttt{ExportTest.php} & S \\ \hline
    ...
    \testI  & Verificare che POST /api/notes/import restituisca 400 con messaggio "mdma format is not supported" quando il file caricato ha un formato non supportato - verificato da \texttt{ImportTest.php} & S \\ \hline
    ...
    \testI & Verificare che POST /api/llm restituisca 200 con il risultato generato dal modello per l'azione "summarize" - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 200 con il risultato generato dal modello per l'azione "rewrite" con opzioni di stile - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 200 con il testo tradotto per l'azione "translate" con lingua di destinazione valida - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 422 con errore di validazione quando viene fornita un'azione non valida - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 422 con errore di validazione quando vengono fornite opzioni non valide - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 422 con errore "Translation in al is not supported" quando viene fornita una lingua non supportata per la traduzione - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 422 con errore di validazione quando viene fornito uno stile di riscrittura non valido - verificato da \texttt{LlmTest.php} & S \\ \hline
    \testI & Verificare che POST /api/llm restituisca 502 con messaggio di errore quando il servizio LLM risponde con un errore interno - verificato da \texttt{LlmTest.php} & S \\ \hline
...
\end{figure}
Dal grafico si osserva l'andamento dell'EAC nei dieci sprint. Nel terzo periodo il valore si mantiene al di sopra del BAC, indicando una tendenza al superamento del budget pianificato; tuttavia, la traiettoria mostra un progressivo avvicinamento alla linea di base, suggerendo un miglioramento nella gestione delle risorse. Nel settimo e ottavo periodo invece si nota uno scostamento del valore EAC oltre il limite inferiore, dovuto ad un aumento dell'efficienza da parte del gruppo. Nel decimo periodo il gruppo ha riorganizzato la distribuzione delle ore, riducendo quelle inizialmente previste per ruoli di responsabile e analista a favore di amministratore e programmatore, pur rispettando i vincoli orari stabiliti. Questa riallocazione ha comportato una diminuzione del preventivo complessivo del progetto. Al termine del decimo sprint, il progetto risulta comunque rientrato nei limiti accettabili (±5\% BAC), condizione da mantenere anche nei periodi successivi.
...
Il suo scopo è assicurarsi che tuttte le istruzioni nel codice vengano testate almeno una volta. \\
Di seguito si riporta un immagine rappresentante la percentuale di Statement Coverage realtiva a quelle che Codecov considera come \textit{"relevant lines"}, per ciascun file contenente codice coperto da test.
\begin{figure}[H]
    \centering
