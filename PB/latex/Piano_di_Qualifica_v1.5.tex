...
\begin{longtable}{|l|p{4cm}|p{8cm}|p{1cm}|}
    ...
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca la nota corrispondente all'ID specificato nel formato richiesto qualora esso sia valido - verificato da \texttt{ExportControllerTest.php} & S \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 404 qualora la nota non sia trovata - verificato da \texttt{ExportControllerTest.php} & S \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 400 qualora l'ID non sia valido - verificato da \texttt{ExportControllerTest.php} & S \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 400 qualora il formato richiesto non sia valido - verificato da \texttt{ExportControllerTest.php} & S \\ \hline
    \testU & ExportController & Verificare che invoke(Request \$request, string \$id) restituisca una risposta JSON con status 500 in caso di errore sconosciuto durante l'esportazione - verificato da \texttt{ExportControllerTest.php} & S \\ \hline
